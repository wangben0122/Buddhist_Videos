package com.arkui.fz_tools.pop;
...
public class BubblePopupWindow extends PopupWindow {
    ...
    public void show(View parent, int gravity, float bubbleOffset) {
        ...
        if (!this.isShowing()) {
            ...
            switch (gravity) {
                case Gravity.BOTTOM:
                    // showAsDropDown(parent,-getMeasuredWidth()+70,0);
                  /*  showAtLocation(parent,Gravity.TOP | Gravity.LEFT,
                            location[0] - getMeasuredWidth(), location[1] + getMeasureHeight());*/
                    int widthPixels = context.getResources().getDisplayMetrics().widthPixels;
                    int px=widthPixels - location[0] - parent.getWidth();//剩余空间
                    showAsDropDown(parent, -(getMeasuredWidth()-(widthPixels-location[0])-px/2), 0);
                    break;
                case Gravity.TOP:
